...
function HeadSection() {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        // Fetch images
        const imageRes = await fetch(`${API_BASE}/api/upload/files`);
        ...
        if (mounted) {
          if (homeBg2) {
            ...
            setBgUrl(bgUrlPath.startsWith('http') ? bgUrlPath : `${API_BASE}${bgUrlPath}`);
          }

          setImages({
            indomie: indomie ? (indomie.url.startsWith('http') ? indomie.url : `${API_BASE}${indomie.url}`) : "",
            spoon: spoon ? (spoon.url.startsWith('http') ? spoon.url : `${API_BASE}${spoon.url}`) : "",
            eatWin: eatWin ? (eatWin.url.startsWith('http') ? eatWin.url : `${API_BASE}${eatWin.url}`) : "",
            chicken: chicken ? (chicken.url.startsWith('http') ? chicken.url : `${API_BASE}${chicken.url}`) : "",
          });
        }
